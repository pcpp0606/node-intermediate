...
import { updateResumeValidator } from '../middlewares/validators/update-resume-validator.middleware copy.js';
...
resumesRouter.get('/:id', async (req, res, next) => {
    try {
        ...
        let data = await prisma.resume.findUnique({
            // where: { id, authorId }, id를 string 값으로 가져오기에 수정해줘야 한다.
